...
// Run with a distinct global shelf (globalSkillsDir() is a module-load const):
//   MM_GLOBAL_SKILLS_DIR=$(mktemp -d) MEMORY_DIR=$(mktemp -d) MM_STATE_DIR=$(mktemp -d) bun run scripts/native-fit-boundary.ts
import { writeSkill, agentSkillsDir, globalSkillsDir, MM_TAG, autonomousShelves } from "../mods/core";
...
writeSkill(globalSkillsDir(), "global-stale", managed("global-stale")); // SHARED global, stale, managed, lexically-prunable
...
console.log("global shelf:", globalSkillsDir(), "(distinct:", agent !== globalSkillsDir(), ")");
console.log("autonomousShelves:", autonomousShelves({}));
const res = runAutonomousPrune({}, { maxRetire: 5 });
console.log("retired:", res.retired);
const globalRemains = existsSync(join(globalSkillsDir(), "global-stale", "SKILL.md"));
