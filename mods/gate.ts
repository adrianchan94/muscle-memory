...
export function sotaQualityGaps(d: { name: string; description: string; body: string }): string[] {
  ...
  if (procedural && !concreteFence) gaps.push('CONCRETENESS: add a fenced example with a real command, file, code fragment, or diff (show the exact correct fix, never hand-wave)');
