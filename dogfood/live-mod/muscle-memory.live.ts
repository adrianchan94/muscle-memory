...
// ── ERROR SEMANTICS LAYER (ESL v0) — added by Scottie 2026-08-12 ──────────────
// WHY: measured across 982 Keystone tool returns, `status:"success"` carried error-like text in
// 27.7% (luna) / 25.0% (terra) / 10.6% (grok) / 7.5% (glm) / 5.4% (sonnet) / 2.7% (k3) of returns,
// while explicit status="error" ranged 0%..13.2%. MM trusted `status` first, so on our primary
// lanes MM was wrong about whether the world failed roughly a quarter of the time — and it wrote
// skill credit, scars and outcomes from that. This normalizes BEFORE any ledger write.
// Returns {status:"ok"|"error"|"unknown", source, retryable, evidence}.
// HIGH-PRECISION ONLY: patterns here must be things that cannot appear in a healthy result.
// A false "error" wrongly penalises a skill, which is worse than a missed one.
var ESL_ERR = [
  [/(?:^|\n)\s*Exit code:\s*([1-9]\d*)/, "exit_code"],
  [/Process exited with code\s*([1-9]\d*)/i, "process_exited"],
  [/Traceback \(most recent call last\)/, "python_traceback"],
  [/(?:^|\n)[^\n]{0,80}: command not found/, "command_not_found"],
  [/No such file or directory/, "enoent"],
  [/Permission denied/, "eperm"]
];
function eslNormalize(event) {
  var out = String(event?.output ?? event?.resultText ?? "");
  var status = String(event?.status ?? "");
  var hasLifecycle = !!(status || event?.ok !== undefined || event?.isError !== undefined || event?.error);
  var evidence = [];
  for (var i = 0; i < ESL_ERR.length; i++) {
    var m = ESL_ERR[i][0].exec(out);
    if (m) evidence.push({ kind: ESL_ERR[i][1], at: m.index, sample: String(m[0]).slice(0, 120) });
  }
  var rawOk = status ? status === "success" : (event?.ok ?? !(event?.isError || event?.error));
  // explicit provider failure always wins
  if (status && status !== "success") return { status: "error", source: "provider_status", retryable: false, evidence: evidence, raw_ok: false };
  if (event?.isError || event?.error) return { status: "error", source: "provider_flag", retryable: false, evidence: evidence, raw_ok: false };
  // provider says fine, body says otherwise
  if (evidence.length) return { status: "error", source: "body", retryable: true, evidence: evidence, raw_ok: rawOk };
  if (!hasLifecycle && !out) return { status: "unknown", source: "missing_lifecycle", retryable: false, evidence: [], raw_ok: rawOk };
  return { status: "ok", source: status ? "provider_status" : "inferred", retryable: false, evidence: [], raw_ok: rawOk };
}
function eslOk(event) {
  var n = eslNormalize(event);
  if (n.status !== (n.raw_ok ? "ok" : "error")) {
    try { appendUiEvent({ phase: "esl_disagree", esl: n.status, raw_ok: n.raw_ok, source: n.source,
                          tool: event?.toolName ?? null, conv: event?.conversationId ?? null,
                          evidence: n.evidence.slice(0, 3) }); } catch {}
  }
  return { ok: n.status === "ok", norm: n };
}
...
function observeToolStart(event, now = Date.now()) {
  ...
  const callId = String(event?.toolCallId ?? "").split("|")[0];
...
function observeToolEnd(event, openPossessions, now = Date.now()) {
  ...
  const __esl = eslOk(event); const ok = __esl.ok;
  if (!ok)
    return null;
  ...
  if (letta.capabilities?.events?.tools) {
    ...
    disposers.push(letta.events.on("tool_start", (event) => {
      try {
        ...
        if (!tool)
          return;
        // ── POSSESSION GATE (v2 spine, 2026-08-12): govern BEFORE the side effect, at the only
        // seam measured to exist on every lane (tool_end never fires for Bash: letta.js executeTool
        // skips non-string returns). return {result} blocks 3/3, return {args} rewrites 3/3 —
        // measured on sonnet-5/Bash AND luna/exec_command. Flag-gated, default OFF.
        if (String(process.env.MM_GATE || "").toLowerCase() === "on") {
          try {
            const ga = event?.args ?? {};
            const gcmd = String(ga.cmd ?? ga.command ?? ga.file_path ?? ga.path ?? "");
            const grule = String(process.env.MM_GATE_BLOCK_RE || "");
            if (grule && gcmd && new RegExp(grule, "i").test(gcmd)) {
              appendUiEvent({ phase: "intervention", kind: "gate_block", seam: "tool_start",
                conv: event?.conversationId ?? null, tool, toolCallId: event?.toolCallId ?? null,
                rule: grule.slice(0, 120) });
              return { result: { status: "success", output:
                "[muscle-memory] This call was BLOCKED before execution by an act-time gate (rule: "
                + grule.slice(0, 120) + "). State that it was blocked and why, then choose a safer alternative." } };
            }
          } catch {}
        }
        ...
        if (applyNudgeEnabled()) {
          try {
            ...
            const probe = String(a.file_path ?? a.path ?? a.cmd ?? a.command ?? "");
    ...
    try {
      disposers.push(letta.events.on("tool_end", (event) => {
        ...
        try {
          ...
          const blockedByGate = outText.includes("[muscle-memory] This call was BLOCKED");
          try {
          ...
          if (autoRateEnabled()) {
            try {
              ...
              if (sk2 && cid2 && !autoRated.has(cid2) && !blockedByGate) {
                autoRated.add(cid2);
                recordPlusMinus(sk2, !!ok, cid2, "observed");
              }
            } catch {}
          }
          appendJsonl(OUTCOME_PATH, { ts: Date.now(), id: event?.toolCallId ?? null, tool: event?.toolName ?? null, conv: event?.conversationId ?? null, ok, err, ...errMsg ? { errMsg } : {}, ...blockedByGate ? { blocked: true } : {} });
...
` + applyNudge(skillName, loadPlusMinus()[skillName])
        ...
        try {
          const __mmKind = coached ? "coached" : closePrompt ? "close_prompt" : applyMsg ? "apply_nudge" : shelfNudge ? "shelf_nudge" : null;
          if (__mmKind)
            appendUiEvent({ phase: "intervention", kind: __mmKind, seam: "tool_end", conv: event?.conversationId ?? null, agent: event?.agentId ?? null, tool: event?.toolName ?? null, toolCallId: event?.toolCallId ?? null });
        } catch {}
